...
public class SCAproxOnlineAlgorithm extends SCOnlineAlgorithm {
	private Random rand;
	
	public SCAproxOnlineAlgorithm(HashMap<Integer, Task> tasks, HashMap<Integer, Worker> workers, Grid grid) {
	...
	protected Worker AssignTask(Task task) {
		...
		for (Worker w : availableWorkers) {
			...
			if (taskOrder != null && rand.nextDouble() < 0.5)  {
